...
for j = 1:num_sample
    ...
    success_rate_str = strcat('Success', num2str(mycount), '/', num2str(num_sample));
    plot(xcoord(1, j),ycoord(1, j), 'o', 'MarkerSize', 2, 'MarkerFaceColor', 'r')
    htextbox = text(-0.1055, 0.069, success_rate_str,'FontSize',10);
%     scatter(xcoord(1, j),ycoord(1, j), sz, c, 'filled');
    drawnow;
    im = frame2im(getframe(1));
    [imind,cm] = rgb2ind(im,256);
    outfile = 'success_rate_animation.gif';
    if j==1
