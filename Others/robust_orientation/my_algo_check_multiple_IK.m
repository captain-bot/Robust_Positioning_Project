...
wrst_ang = [NaN];
included_ang_array = zeros(1,size(ik_array,1));
valid_ang_array = [];
valid_ik_indx = [];
for sol_num = 1:size(ik_array,1)
    % Generate uniform random numbers in the joint range
    solIKs = ik_array(sol_num,:);
    ...
    if ~isnan(q_opt)
        ...
        included_ang_array(sol_num) = included_ang*180/pi;   % angles in degrees
%         fprintf('-----------------------------------\n');
    else
    ...
    indx_lower = find(proj_len_array < abs(proj_len_optimal));  % considering absolute value because of the double cover nature of quaternion
    ...
    %/////////////////////////////////////////////////////////
    if success_rate > 95
        valid_ang_array = [valid_ang_array,included_ang*180/pi];
        valid_ik_indx = [valid_ik_indx,sol_num];
    end    
end