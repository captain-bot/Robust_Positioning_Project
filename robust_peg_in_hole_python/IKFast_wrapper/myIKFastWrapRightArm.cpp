...
#include "baxter_right_arm_ikfast_solver.cpp" /*use right_arm solver if working with right arm*/
#include <stdio.h>
#include <stdlib.h>
...
extern "C"{
	...
	for(int i = 0; i < numListElement; i++) {
		...
		if (got_sol) {
		 ...
		 w1_ang += dang; /* increase w1_ang */
        }
        if (num_of_valid_solutions != 0) {
		std::cout << "IKFast found " << num_of_valid_solutions << " valid IKs\n" << std::endl;
		return Py_True;
        }
        else {
  ...
  static struct PyModuleDef ikModuleRight = {
  ...
  PyMODINIT_FUNC PyInit_ikModuleRight(void) {
	import_array();
  	return PyModule_Create(&ikModuleRight);
  }
}
